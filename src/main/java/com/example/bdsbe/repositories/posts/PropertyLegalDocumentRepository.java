...
@Repository
public interface PropertyLegalDocumentRepository
    extends JpaRepository<PropertyLegalDocument, Long> {

  @Query(
      "SELECT p FROM PropertyLegalDocument p "
  ...
  Page<PropertyLegalDocument> filter(PropertiesFilter filter, Pageable pageable);
}
